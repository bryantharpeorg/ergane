...
def _validate_command(args: argparse.Namespace) -> int:
    ...
    #     It runs after the resolution layer because it is the expensive tier:
    #     resolution only opens files, this one parses each module's AST.
...
def _all_pass_phrases(checked: list[str]) -> list[str]:
    ...
    and `symbol_anchors` are appended only when those layers actually ran and
    did not skip, and in the order they run, so the sentence reads as the
    sequence the operator's spec was actually put through.
    """
    phrases = [
        "frontmatter",
