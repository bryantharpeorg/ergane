"""The two anchor layers 072 added, relocated from the CLI module (133-US2).

`anchor_resolution` (072-US1) opens every `path:NN` citation in the authored
documents against the target repository, and `symbol_anchors` (072-US2) resolves
cited Python symbols against their declared spans. Their bodies moved here
byte-for-byte: every refusal string and every parameter is unchanged, and both
checkers still accumulate into caller-owned `findings`, `skipped` and `checked`
lists rather than returning a report — the one signature redesign this
relocation is forbidden to make (plan trap 7).

The import direction is one-way (plan trap 17): nothing here imports from
`factory.cli.nouns.spec`. The two module-level regexes below travelled with the
bodies that alone read them; a moved body that needs a CLI-module name means
that name moves too.
"""
...
# The module-level grammars the CLI module bound above every name it defined
# (`factory/cli/nouns/spec.py:67-71` at 602a92c), moved verbatim with their `#:`
# comments because the checker below reads them and the import back is one-way.
