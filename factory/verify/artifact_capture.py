...
def _refusal(
    ...
    *,
    ...
    if error is not None and error.errno == errno.ENOENT:
...
def _observation_error(error: OSError) -> SourceObservation:
    if error.errno == errno.ENOENT:
        return _refusal("", error=error)
    if error.errno == errno.ELOOP:
        return _refusal("final path component is a symlink")
    return _refusal("source path could not be opened safely", error=error)
...
def observe_source(
    ...
    *,
    ...
    The worktree root must already be explicit and absolute.  Directory and file
    components are opened with `O_NOFOLLOW`, the final file with `O_NONBLOCK`,
    and an actual regular file is checked by descriptor rather than by its
    lexical manifest spelling.
    """
    try:
...
def _compare_provenance(
    ...
    if before.status is SourceStatus.ABSENT:
        if after.status is SourceStatus.PERMITTED or after.status is SourceStatus.OVERSIZED:
    ...
    if before.status is SourceStatus.PERMITTED:
        ...
        return CaptureProvenance.CHANGED
    if (
        before.status is SourceStatus.OVERSIZED
        and after.status is SourceStatus.OVERSIZED
        and before.metadata_digest == after.metadata_digest
    ):
        return CaptureProvenance.UNCHANGED
    return CaptureProvenance.UNKNOWN
...
def capture_source(
    ...
    *,
    ...
    `UNKNOWN`, never a claim that presence means the gate just produced the file.
    A baseline that changed in bytes or descriptor metadata is refused as
    unstable, even when the captured read itself was otherwise safe.
    """
    ...
    if (
        provenance is CaptureProvenance.CHANGED
        or baseline is not None
        and baseline.status is SourceStatus.UNSTABLE
    ):
        status = SourceStatus.UNSTABLE
        bounded_bytes = None
        size = after.size
