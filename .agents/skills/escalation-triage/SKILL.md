...
```bash
ls -la .factory/transcripts/<epic>/<story>/          # one dir per attempt, mtime = end
tail -c 2000 .factory/transcripts/<epic>/<story>/attempt-N/stdout.log
```
...
```bash
git -C .factory/worktrees/<epic>/<story> log -3 --oneline    # local worktree
