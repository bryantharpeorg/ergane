...
| `factory/activities/notify_activities.py` | `send_escalation` / `expire_escalation` / `settle_escalation` / `send_question` / `find_ferried_question` plus US4's `record_roadmap_failure` and `reset_roadmap_failures`: the evidence-first notification surface; roadmap failures are recorded before delivery is attempted so a down notifier loses only the message (FR-010). `settle_escalation` (041-US2) is `expire_escalation`'s mirror for the answered direction, so the row settles on the workflow's transition rather than on whichever channel happened to answer (FR-013) |
| `factory/escalation/workflow.py` | `EscalationWorkflow` (041-US2): one human decision per workflow — ID = correlation id, delivery an activity through the US1 messenger adapter, expiry its own durable timer, answer via 008's `escalation_resolved` signal, result `ANSWERED(resolution, identity) \| EXPIRED`. Startable standalone by a caller with no epic or as a child by one that has; settles its own row on every terminal transition; `escalation_status` query is what `ergane escalations list` reads |
| `factory/escalation/client.py` | the client side of the above: `mint_correlation_id` / `start_escalation` (the starter mints the id, because a workflow id must exist before `start_workflow`), and `open_escalations`, which enumerates running `EscalationWorkflow`s through Temporal's visibility index and asks each one what it is waiting for — never a store scrape (FR-008) |
| `factory/supervision/alert.py` | 042-US1: the out-of-band alert, and the one page in the system that is *not* an escalation. `StackAlert(service, condition, duration)` → `render_alert` → 041's messenger seam, directly: no Temporal client, no workflow, no row, no expected reply, a local log as its only record. It exists because an alert saying Temporal is down cannot be hosted by Temporal, which is also why 041 FR-002 made that seam a plain library. A failed send is data — logged, complained about on stderr, `exit_code` non-zero — never an exception, because a supervisor that crashes on a failed send has stopped supervising |
| `factory/supervision/units.py` | 042-US2: the systemd user units, generated rather than shipped, because a static file cannot name the operator's install path. `resolve_layout` → `generated_files` → `install` / `uninstall`, with `ergane worker install` as the noun (`factory/cli/nouns/worker.py`). Carries three lessons from a host outage: `KillMode=control-group` so stopping a unit takes agents and their spawned test servers with it, a slice bounding memory and tasks so a leak cannot take the host, and a bounded restart rate so a flap cannot deepen an OOM storm. The probe's own unit and timer belong to 042-US4 with the probe itself: generated ahead of it they would name a module that does not exist yet, and Python's exit 1 on `ModuleNotFoundError` sits inside the `SuccessExitStatus=0 1` such a unit declares — the timer would read green while supervising nothing. Provenance is a digest recorded at install time, so uninstall removes exactly what it wrote and reports rather than deletes a same-named unit it did not; the in-flight-epic refusal is a read through the roadmap's own capacity query, imported lazily so the probe's import graph stays free of Temporal |
| `factory/supervision/probe.py` | 042-US4: the timer's process — unit liveness, host memory headroom, reachability, and the orphaned-test-server reap that is the one thing it does which systemd cannot. One `Host` seam carries every reading, which is what lets the 2026-08-11 orphan class be tested against a *string* rather than a seeded population of the processes that killed the host. It reports and never remediates (FR-014, asserted over its own executable source); alerts are edge-triggered under a heartbeat; an alert nobody received exits 2, outside the `SuccessExitStatus=0 1` its unit declares |
...
| `factory/worker.py` | runnable `python -m factory.worker` — registers `EpicWorkflow` + `RoadmapWorkflow` (D-031) + `EscalationWorkflow` (041-US2) plus every component's activities |
