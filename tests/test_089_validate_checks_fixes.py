...
def _validate_without_fixes_layer(
    ...
    """Run validate with the `_check_fixes` layer monkeypatched to a no-op.
    ...
    original = _composition_module._check_fixes
    _composition_module._check_fixes = lambda *args, **kwargs: None
    try:
        result = run(
            "spec",
            "validate",
        ...
        _composition_module._check_fixes = original
