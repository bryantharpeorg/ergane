...
def test_the_schema_version_is_recorded_once(store: sqlite3.Connection) -> None:
    ...
    # 7 since 068-US2 widened `escalations.resolution` to admit `KILL_EPIC`.
    # The literal is here on purpose: a bump is a claim that every existing
    # store has a migration path, and `tests/test_escalation_record.py` is
    # where that claim is checked against a store built in the previous shape.
