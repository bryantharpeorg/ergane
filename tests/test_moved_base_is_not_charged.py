...
async def test_a_node_whose_siblings_land_forever_eventually_stops(
    ...
    assert states(status) == {"us1": NodeState.KILLED}
    assert len(script.escalation_requests) == 1
    # 079-US1 (FR-002): both bounds are spent by the time this page is raised —
    # the free rebase and the one charged cycle — so `RETRY` is not on it. It
    # was, until this story, and it did nothing but kill the node on the press:
    # the resolution reached `_apply_landing_resolution`, matched no branch and
    # fell through. What the page offers now is what the epic can still do.
