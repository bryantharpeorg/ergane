# Tasks: Stale compiled graph

## Phase 1: User Story 1 — Save a link

- [ ] T001 [P] [US1] Write the failing test: saving a URL stores a link with a
      unique short code (spec US1-S1) — must fail.
- [ ] T002 [US1] Store the link and mint the code.

## Phase 2: User Story 2 — Follow a short link

The story this phase was written for is gone from `spec.md`; the phase survives
so the surviving `us2` node fails on the specification rather than on its task
slice, which is what pins the finding to `spec.md`.

- [ ] T003 [P] [US2] Write the failing test — must fail.
- [ ] T004 [US2] Resolve the code and redirect.

## Verification

- [ ] Final gate command passes green.
