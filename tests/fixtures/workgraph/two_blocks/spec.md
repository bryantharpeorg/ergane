...
**Feature Branch**: `two_blocks`

**Created**: 2026-08-05

**Status**: Draft

**Input**: Deriver fixture — rejection (`section_missing`): the `## Work Graph`
section holds two fenced YAML blocks — a complete graph and a leftover earlier
revision. The first block is valid on its own, so only the two-block rule
distinguishes a correct deriver from one that silently takes the first.
...
An earlier revision, left behind:
