...
**Feature Branch**: `no_section`

**Created**: 2026-08-05

**Status**: Draft

**Input**: Deriver fixture — rejection (`section_missing`): this spec declares no work
graph. The one header of that name it contains is quoted inside a four-backtick fence,
so the fence-masked scan must read past it rather than derive the graph inside it.
...
This spec quotes the grammar it does not use. The block below is fenced, so the
header inside it is prose about a work graph, not a work graph:
