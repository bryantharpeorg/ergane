...
- [ ] T003 [US1] Add US1-S4, US1-S5 usage-source tests in `tests/test_attestation_usage.py`: partial dimensions despite confirmed flag, repeated model amounts with distinct request IDs, gateway/CLI overlap, subscription dollars, ambiguous legacy joins and unknown serving identity.
- [ ] T004 [US1] Implement typed identity and journal contracts in `factory/attestation/`, and additive invocation provenance in `factory/usage/models.py`, `factory/usage/ledger.py` and `factory/activities/usage_activities.py`; preserve old schema/payload reading and authoritative ledger ownership.
