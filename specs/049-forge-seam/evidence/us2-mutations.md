...
facts are still built only by `factory/mergequeue/onboard.py` (FR-008).
...
and after), reverted with `git checkout HEAD --`, run over the six suites this
story touches. Each edit is the one its test's docstring names.

    85 passed in 1.76s          # the same six suites, unmutated

| edit | tests red |
|---|---|
| M1 the visibility check goes back into the judgment | 16 · no-visibility, source scan, +14 |
...
| M8 the title remedy stops reaching the finding | 2 · landing_title, squash-title |
...
what "the same remedy an operator reads today" (US2-S2) means: one word, red.
