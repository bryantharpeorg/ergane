...
- [ ] T002 [US1] Verify prerequisites in this worktree: `uv run pytest -q` is
      green and the plan's reuse points exist where it says they do — constitution
      I gate; if the plan's inventory is wrong, STOP and write the discrepancy
      under the operator-question marker (this epic's own mechanism, used
      manually until it exists).
- [ ] T003 [P] [US1] Write marker-detection cases FIRST: a final message with the
      ...
      quotes it in a code block does not; an empty body under the marker is a
      malformed question and classifies as today's FAIL (a question with no
      content is not a question) — must fail.
- [ ] T004 [P] [US1] Write ladder-routing cases FIRST in the interpreter tests:
      ...
      attempt — must fail.
- [ ] T005 [US1] Write delivery cases FIRST against the fake bridge: the question
      ships once, attributed epic/node/attempt, body verbatim; the credential
      sweep assertion extends to the question payload (FR-007) — must fail.
...
- [ ] T008 [US1] Implement question delivery through the escalation send with a
      question kind, until T005 passes. Extend the prompt contract text (the
      assembler's standing agent instructions) to state the marker and the bar:
      a question names its fork and the options considered, and is for genuinely
      blocking forks only.
...
- [ ] T009 [P] [US2] Write bridge reply-routing cases FIRST: a reply to a
      question message resolves that escalation with the reply text; with two
      questions open, each reply routes by reply-to threading, never by recency
      (FR-008); a non-reply message and a reply to a non-question message are
      both ignored — must fail.
...
- [ ] T011 [US2] Add the `MessageHandler` reply path to
      `factory/notify/service.py`, storing send-time message-id → escalation-id
      mapping, until T009 passes. No new signal: resolution rides
      `escalation_resolved` with the answer text as its string payload.
...
- [ ] T013 [US2] Final sweep + docs: extend the credential sweep to answers and
      ...
      (questions are now a first-class kind) and note the marker contract where
      agent prompt contracts are documented.
