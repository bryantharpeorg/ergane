...
:735). `adapter_for(agent, **options)` (`factory/workgraph/adapter.py:750`) looks the name up in
`_ADAPTERS` and **already raises `AdapterError`** for an unknown name, with the
known-agent list in its message (`factory/workgraph/adapter.py:757-764`). The registry itself is a
single-entry dict: `_ADAPTERS = {ClaudeCodeAdapter.name: ClaudeCodeAdapter}`
(`factory/workgraph/adapter.py:1512`).
...
called as `adapter_for(DEFAULT_AGENT)` (`factory/activities/agent_activities.py:521`). The comment
above the constant (`factory/activities/agent_activities.py:175-177`) claims `AttemptContext`
...
the guard at `factory/workgraph/adapter.py:750` is unreachable from production.
...
(`factory/config.py:151-152`). So `route:` is refused outright today and must be added
...
(`factory/workgraph/adapter.py:993`) `run_attempt` begins at :1022; its per-CLI `argv()` method
follows at the launch section (`factory/workgraph/adapter.py:1195` region). The shared policy —
...
1. Add `route` to `_OPTIONAL_FIELDS` (`factory/config.py:152`).
...
4. Re-point the three predicates (`factory/config.py:197-216`) to single-axis reads:
   `is_llm` on `agent != none`; `routes_through_gateway` and
   `needs_virtual_key` on `route == gateway`.
5. Re-point `route_of` (`factory/verify/models.py:837`) to read the field.
...
The check exists at `factory/workgraph/adapter.py:750`; this story makes the load path reach it.

1. In `load_personas`, after the non-empty-string guard (`factory/config.py:268-270`),
...
2. Exempt the deterministic persona (`agent: none`) — no CLI, no adapter, must
   stay loadable.
3. The registry of known agents is `_ADAPTERS` (`factory/workgraph/adapter.py:1512`). Read it,
...
1. Change `factory/activities/agent_activities.py:521` from `adapter_for(DEFAULT_AGENT)` to
   `adapter_for(<the persona's agent>)`, sourced from the attempt context that
   already carries it (`factory/workgraph/models.py:320`).
2. Correct the stale comment at `factory/activities/agent_activities.py:175-177` in the same
   change.
...
1. Extract the shared policy out of `ClaudeCodeAdapter.run_attempt`
   (`factory/workgraph/adapter.py:1022-1191`) into one implementation both adapters call.
2. Define the narrow inner seam as the per-CLI surface only: `argv`, prompt
   ...
   `specs/005-workgraph-interpreter/contracts/adapter.md:10-14` sketched and the
...
3. Parametrize the adapter conformance tests over `_ADAPTERS`
   (`factory/workgraph/adapter.py:1512`), not over the literal `claude-code`.
...
- **Trap 3 — `route_of` derives today; it must read.** `factory/verify/models.py:837`
...
- **Trap 6 — the unknown-agent refusal must come from the registry, not a new
  literal.** `_ADAPTERS` at `factory/workgraph/adapter.py:1512` is the source of truth for "no
