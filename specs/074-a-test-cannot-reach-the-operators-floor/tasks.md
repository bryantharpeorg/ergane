...
- [ ] T001 [P] [US1] (spec US1-S1, FR-004) In
      ...
      three.
- [ ] T002 [P] [US1] (spec US1-S3, FR-005) **The control for trap 7.** Assert the
...
- [ ] T003 [P] [US1] (spec US1-S5, FR-001) Assert the seam declared beside the
      choke point is bindable by exactly the name the refusal of T001 prints.
- [ ] T076 [P] [US1] (spec US1-S6, FR-002, trap 19) **The landed guard.** Assert
...
- [ ] T079 [US1] (spec US1-S7, FR-027, trap 23) **The landed floor-fakes.**
      ...
      clause alone.
- [ ] T080 [P] [US1] (spec US1-S8, FR-029, trap 24) **The CLI guard sweep.** In
...
- [ ] T005 [US1] (spec US1-S4, FR-022) **Trap 10.** In
...
- [ ] T009 [P] [US1] (FR-002, trap 19) Migrate
...
- [ ] T010 [P] [US1] (FR-002, trap 19) Migrate
...
- [ ] T011 [P] [US1] (FR-002, trap 19) Migrate `factory/cli/repo.py:85` —
...
Waits for Phase 1 **and** Phase 3 to merge. Every task below edits the module
Phase 1 creates, and a pass edge would only have waited for its verdict — the base
pin would hold no choke point to harden. Phase 3 supplies the opt-in name FR-009
wires to.
...
Independent of Phases 1, 2, 4, 5 and 6.
...
- [ ] T077 [P] [US3] (spec US3-S7, FR-010, trap 21) **Where the opt-in lives.**
...
- [ ] T027 [US3] (spec US3-S5, FR-014) **The enumerating test.** Walk every
...
- [ ] T028 [P] [US3] (spec US3-S6, FR-025) Assert every `live_*` registration at
...
- [ ] T029 [US3] (FR-010, trap 21) Add the named opt-in and its parser. One name
      ...
      a pytest plugin.
- [ ] T030 [US3] (FR-011, FR-012) Convert `tests/test_live_notify.py` — change
...
- [ ] T031 [P] [US3] (FR-011, FR-012) Convert the `live_proxy` tier, including
      `tests/test_live_judge.py`, which carries that marker rather than one of its
      own.
- [ ] T032 [P] [US3] (FR-011, FR-012) Convert the `live_epic` tier.
- [ ] T033 [P] [US3] (FR-011, FR-012) Convert the `live_merge` tier.
- [ ] T034 [P] [US3] (FR-011, FR-012) Convert the `live_capacity` tier in
...
- [ ] T035 [P] [US3] (FR-011, FR-012) Convert the `live_onramp` tier — the sixth
      tier, which landed after this spec was drafted and is absent from the
      2026-08-20 task list.
- [ ] T036 [US3] (FR-010, FR-025) Rewrite the six marker registrations at
...
- [ ] T037 [US3] (FR-014) Make T027 pass. **Trap 15**: every tier must still be
...
Independent of Phases 1, 2, 3, 5 and 6.
...
Independent of Phases 1, 2, 4 and 6, and declared `concurrent_with: [US4]`. It
waits for Phase 3 to merge: both stories write `tests/test_live_capacity.py`
(trap 13).
...
## Phase 6: User Story 6 — The last seven callers, and the test that keeps them there

Waits only for Phase 1 to merge.

### Tests for this story (write FIRST, must fail)

- [ ] T059 [P] [US6] (spec US6-S1, FR-003) In
...
- [ ] T060 [US6] (spec US6-S2, FR-023) One case per migrated caller, seven small
...
- [ ] T062 [P] [US6] (spec US6-S4, FR-006) Assert that merely importing
      ...
      the caller.
- [ ] T078 [P] [US6] (spec US6-S5, FR-023, trap 19) **The landed guard.** Walk the
...
- [ ] T081 [US6] (spec US6-S6, FR-027, FR-028, trap 23) **The landed
...
- [ ] T075 [US6] (FR-023, trap 18, trap 19) **Trap 18.** Migrate
      ...
      that assert on it.
- [ ] T069 [US6] (FR-003) Make T059 pass by leaving no module but the choke point
...
- [ ] T070 [US6] (spec US6-S1, FR-003) Paste, as committed evidence in the test
