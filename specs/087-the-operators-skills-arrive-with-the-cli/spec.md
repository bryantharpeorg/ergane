...
1. **Given** the tracked `.agents/skills/` source, **When** a wheel and sdist are built, **Then** every regular file and safe in-repository symlink beneath each declared skill directory is present at the same relative path in package resources, including scripts and references — proven by archive-content tests derived from the source tree.
...
3. **Given** a destination file, directory, or alias differs and is not owned by this installation, **When** install runs, **Then** the collision is preserved, reported with its exact path, and no sibling skill is rolled back or overwritten — proven by a truth-table test.
...
4. **Given** the wider `ergane uninstall` flow, **When** its survey and perform phases run, **Then** skill actions participate in the existing preview/ownership contract and do not become an unreviewed recursive delete — proven by survey/perform integration tests.
...
- **FR-004**: Release validation MUST compare source and archive path/digest inventories without a fixed skill or file count.
...
- **FR-009**: Unowned or modified collisions MUST be preserved and reported; they MUST NOT block unrelated safe entries.
...
- **FR-014**: Modified owned paths and unrelated entries MUST be preserved with reviewable evidence.
