...
You were dispatched with an assembled prompt and a standards path named by
`ergane.yaml`. Those bind you; this page does not. It is written for an operator
session — an agent working *on* the factory rather than a node working *inside*
it. Nothing here widens, narrows or overrides your task scope. You are reading
it only because it sits on an ancestor path of your worktree. Follow your prompt
and the standards file it names.
...
- `.specify/memory/constitution.md` — **normative**. `ergane.yaml` names it as the
...
`ergane.yaml` is the active manifest: it declares this repository's runtime, its gates and
the standards path named above. `factory.yaml` is the legacy name, retired for the file
and still honored by code that reads it, so historical documents that say `factory.yaml`
are describing a name that was live when they were written — leave landed history as it
stands and say `ergane.yaml` for anything new.
...
Reading this repository's state — the commands above, a spec, a finding, a ledger row,
a workflow's status — is an observation, and an observation request authorizes no
mutation. Answering "what does 157's status say" does not become "so fetch, so
merge, so dispatch": a status or diagnosis request carries none of that authority,
and the recipes for those live nowhere in this page.

A fetch, a merge, a dispatch, an attestation, an answer to an escalation, a findings
application, a service change, a commit, a push or a publication is an action, and
every action requires its own declared intent — asked for, and granted, separately.
The live-state table above is the full authority an observation carries.
...
A second standards channel. Implementer agents already have one, named by `ergane.yaml`
and enforced at dispatch, and it was chosen deliberately so that nothing depends on a
client auto-loading this page. Requirements for how code is written go there. This page
orients an operator session and nothing else, and `tests/test_operator_instructions.py`
holds it to that: every command it names must resolve, every path it cites must exist,
and it may not state a status that a live source already answers.